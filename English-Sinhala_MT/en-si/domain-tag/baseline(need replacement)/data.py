...
class Vocab(object):
    ...
    def __init__(self):
        ...
        self.add_symbol("<sos>")
        self.add_symbol("<sos_bad>")  # Start of sentence token for tgt=si
        self.add_symbol("<sos_good>")  # Start of sentence token for tgt=hi
...
# original MTDataset, changed <sos> to <sos_nso>, used for loading validation dataset
class MTDataset_valid(data.Dataset):

    def __init__(self, vocab, prefix, src_lang="en", tgt_lang="si"):
        # Attributes
        self.vocab = vocab
        ...
        # Append start tag of sentence token to the source (different target have different sos)
        for idx, src_sent in enumerate(self.src_txt):
            self.src_txt[idx] = f"<sos_good> {src_sent}"
        # Convert to indices
        self.src_idxs = [
...
class MTDataset_multi(data.Dataset):

    def __init__(self, vocab, prefix_1, prefix_2, src_lang="en", tgt_lang_1="si", tgt_lang_2="hi"):
        # Attributes
        self.vocab = vocab
        self.src_lang = src_lang
        self.tgt_lang_1 = tgt_lang_1
        self.tgt_lang_2 = tgt_lang_2
        # Load from files, src1-tgt1, src2-tgt2
        src_file_1 = prefix_1 + "." + src_lang
        src_file_2 = prefix_2 + "." + src_lang
        tgt_file_1 = prefix_1 + "." + tgt_lang_1
        tgt_file_2 = prefix_2 + "." + tgt_lang_2
        self.src_txt_1 = loadtxt(src_file_1)
        self.src_txt_2 = loadtxt(src_file_2)
#        self.src_txt = self.src_txt_1 + self.src_txt_2
        self.tgt_txt_1 = loadtxt(tgt_file_1)
        self.tgt_txt_2 = loadtxt(tgt_file_2)
        
        
        # Append tag to identify the target language in src_txt
        for idx, src_sent in enumerate(self.src_txt_1):
            self.src_txt_1[idx] = f"<sos_bad> {src_sent}"
        for idx, src_sent in enumerate(self.src_txt_2):
            self.src_txt_2[idx] = f"<sos_good> {src_sent}"
        self.src_txt = self.src_txt_1 + self.src_txt_2

        # Append start/end of sentence token to the target
        for idx, tgt_sent in enumerate(self.tgt_txt_1):
            self.tgt_txt_1[idx] = f"<sos> {tgt_sent} <eos>"
        for idx, tgt_sent in enumerate(self.tgt_txt_2):
            self.tgt_txt_2[idx] = f"<sos> {tgt_sent} <eos>"
        self.tgt_txt = self.tgt_txt_1 + self.tgt_txt_2
        # Check length
        self.length = len(self.src_txt)
        if self.length != len(self.tgt_txt):
            raise ValueError("Mismatched source and target length")
