...
class MTDataset_valid(data.Dataset):
    ...
    def __init__(self, vocab, prefix, src_lang="en", tgt_lang="fr"):
        ...
        # Append start/end of sentence token to the target (different target have different sos)
        for idx, tgt_sent in enumerate(self.tgt_txt):
            self.tgt_txt[idx] = f"<sos_good> {tgt_sent} <eos>"
        # Convert to indices
        self.src_idxs = [
...
class MTDataset_multi(data.Dataset):
    ...
    def __init__(self, vocab, prefix_1, prefix_2, src_lang="en", tgt_lang_1="si", tgt_lang_2="hi"):
        ...
        for idx, tgt_sent in enumerate(self.tgt_txt_1):
            self.tgt_txt_1[idx] = f"<sos_bad> {tgt_sent} <eos>"
        for idx, tgt_sent in enumerate(self.tgt_txt_2):
            self.tgt_txt_2[idx] = f"<sos_good> {tgt_sent} <eos>"
