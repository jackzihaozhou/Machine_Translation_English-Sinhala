...
import torch as th
from data import MTDataset_valid, MTDataLoader, Vocab, MTDataset_multi
...
def load_data(src_lang, tgt_lang, cached_folder="assignment2/data", overwrite=False):
    ...
    if not os.path.isfile(cached_file) or overwrite:
        ...
        # used for multi-language (another language: hi)
        multi_prefix = os.path.join(
            multi_folder,
            f"{src_lang}hi_parallel.bpe.train"
        )
        ...
        train = MTDataset_multi(vocab, train_prefix, multi_prefix,
                          src_lang=src_lang, tgt_lang_1=tgt_lang, tgt_lang_2="hi")
        valid = MTDataset_valid(vocab, dev_prefix,
...
def get_args():
    ...
    parser.add_argument("--tgt", type=str, default="si",
                        choices=["si"])
...
def train_epoch(model, optim, dataloader, lr_schedule=None, clip_grad=5.0):
    ...
    for batch in itr:
        ...
        nll = th.nn.functional.nll_loss(
            ...
            # Target tokens (we start from the 1st real token, ignoring <sos_tgt>)
            tgt_tokens[1:].view(-1),
...
def evaluate_ppl(model, dataloader):
    ...
    for batch in tqdm(dataloader):
        ...
        with th.no_grad():
            ...
            # Number of tokens (ignoring <sos_tgt> and <pad>)
            n_sos_si = tgt_tokens.eq(model.vocab["<sos_bad>"]).float().sum().item()
            n_sos_hi = tgt_tokens.eq(model.vocab["<sos_good>"]).float().sum().item()
            n_pad = tgt_tokens.eq(model.vocab["<pad>"]).float().sum().item()
            n_tokens = tgt_tokens.numel() - n_pad - n_sos_si - n_sos_hi
            # Keep track
            tot_nll += nll.item()
            tot_tokens += n_tokens
