...
def load_data(src_lang, tgt_lang, cached_folder="assignment2/data", overwrite=False):
    ...
    if not os.path.isfile(cached_file) or overwrite:
        ...
        # used for multi-language (another language: dv)
        multi_prefix = os.path.join(
            multi_folder,
            f"{src_lang}dv_parallel.bpe.train"
        )
        ...
        train = MTDataset_multi(vocab, train_prefix, multi_prefix,
                          src_lang=src_lang, tgt_lang_1=tgt_lang, tgt_lang_2="dv")
...
def evaluate_ppl(model, dataloader):
    ...
    for batch in tqdm(dataloader):
        ...
        with th.no_grad():
            ...
            n_sos_si = tgt_tokens.eq(model.vocab["<sos_si>"]).float().sum().item()
            n_sos_dv = tgt_tokens.eq(model.vocab["<sos_dv>"]).float().sum().item()
            n_pad = tgt_tokens.eq(model.vocab["<pad>"]).float().sum().item()
            n_tokens = tgt_tokens.numel() - n_pad - n_sos_si - n_sos_dv
            # Keep track
            tot_nll += nll.item()
            tot_tokens += n_tokens
